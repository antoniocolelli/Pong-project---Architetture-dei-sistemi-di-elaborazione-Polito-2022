...
* Attention		 : None
*******************************************************************************/
void LCD_Clear(uint16_t Color,uint16_t Wall)
{
	uint32_t index;
	int x;
	...
	for(x=0;x<5;x++){
		//x:0->239
		//y:0->319
		LCD_DrawLine(x,0,x,MAX_Y-1,Wall);
		LCD_DrawLine(MAX_X-1-x,0,MAX_X-1-x,MAX_Y-1,Wall);
	}
}
...
* Attention		 : None
*******************************************************************************/
void PutChar( uint16_t Xpos, uint16_t Ypos, uint8_t ASCI, uint16_t charColor, uint16_t bkColor ,uint16_t rovescio)
{
	uint16_t i, j;
...
* Attention		 : None
*******************************************************************************/
void GUI_Text(uint16_t Xpos, uint16_t Ypos, uint8_t *str,uint16_t Color, uint16_t bkColor,uint16_t rovescio)
{
    uint8_t TempChar;
    ...
    {
        TempChar =str[i];  
        PutChar( Xpos, Ypos, TempChar, Color, bkColor,rovescio );    
        if( Xpos < MAX_X - 8 -5 )
        {
            Xpos += 8;
        } 
    ...
    while ( (rovescio && i>=0 ) || (!rovescio && i<x) );
}
